#!/usr/bin/env python3
"""Blender MCP server for caw-agent (Content-Length stdio JSON-RPC)."""
...
from backend import BlenderError, BlenderSession  # noqa: E402
...
def log(msg: str) -> None:
    path = os.environ.get("BLENDER_MCP_LOG")
    line = f"[blender-mcp] {msg}\n"
    if path:
        try:
...
class Server:
    ...
    def ensure_session(self) -> BlenderSession:
        if self.session is None:
            self.session = BlenderSession()
